...
- **override**: default the blacklist to VALUE so it need not guess the label
- **docs**: correct the inverted labels map in the NER detector reference

## 1.6.0 (2026-09-03)

### Feat

- **cli**: add the anonymize command over config, api, or the default detector
- **config**: make linker and anonymizer optional, drop dead dep markers
- **client**: add timeout, headers, and retries to PIIGhostClient
- **api**: lazily re-export the core building blocks from the package root
- **observation**: make clear-text tracing an explicit, warned choice

### Fix

- **detector**: match ASCII digits only in the regex catalogs
- **detector**: harden exact-match, NER, LLM detectors and the LLM guard
- **placeholder**: harden deanonymization, token grammar, and injection
- **pipeline**: resolve overlapping detections by default
- **integrations**: stop leaking clear PII through tool_calls and block content

### Perf

- **pipeline**: memoize thread tokens, batch Redis, bound in-memory store
