...
| Permettre une restauration côté serveur | **Type + id (opaque)** (`<<PERSON:1>>`{ .placeholder }) | Réversible, audit trivial, aucune collision. Built-in `LabelCounterPlaceholderFactory` ou `LabelHashPlaceholderFactory`. |
