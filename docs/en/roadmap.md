...
A document-de-identification web app that runs entirely in the browser, so a regulated professional can de-identify a client file without any data leaving the machine, answers the confidentiality and consent constraints raised repeatedly around client data. The engine already exists, the project website runs the real `piighost` in the browser through Pyodide, with in-browser GLiNER detection, so the library itself needs no reimplementation. What remains is the application around it, client-side document parsing (PDF, DOCX) and OCR, a review step where the user validates or completes the de-identification, and a share step. This is a separate application built on the library, not a library feature.
...
- **Blocking requests or deleting PII.** `piighost` secures PII by detecting and de-identifying it. Whether to refuse a request or erase a value is the caller's policy, decided from the detections `piighost` surfaces, not enforced here.
